...
class ProductsController extends Controller
{
    ...
    public function products($url=null){
    	...
    	$categories = Category::with('childs')->where(['category_parent_id' => 0])->get();

        	$categoryDetails = Category::where(['url'=>$url])->first();
              //dd($categoryDetails->category_parent_id);

        $cat_ids[] = $categoryDetails->id;  
        //dd($categoryDetails->category_parent_id);
    	if($categoryDetails->category_parent_id==0){
    		$subCategories = Category::where(['category_parent_id'=>$categoryDetails->id])->get();
            //dd($subCategories);
            ...
            //dd($cat_ids);
    		$productsAll = Product::whereIn('category_id', $cat_ids)->where('status','Active')->get();

    	}else{

    		$productsAll = Product::where(['category_id'=>$categoryDetails->id])->where('status','Active')->get();	
    	}
    	// dd($productsAll);
      //    exit;
    	return view('frontend.products.listing')->with(compact('categories','productsAll','categoryDetails'));
    }
    ...
    public function product($id = null){
        ...
        $total_stock=10;
        return view('frontend.products.detail')->with(compact('productDetails','categories','productAltImages','total_stock','relatedProducts'));
    }
    ...
    public function addtocart(Request $request){
       ...
       // echo "<pre>"; print_r($data); die;
        ...
        $countProducts = DB::table('carts')->where(['product_id' => $data['product_id'],'product_color' => $data['product_color'],'size' =>'big','session_id' => $session_id])->count();
        if($countProducts>0){
    ...
    public function deleteCartProduct($id=null){
        ...
        return redirect()->route('frontend.cart')->with('flash_message_success','Product has been deleted in Cart!');
    }
