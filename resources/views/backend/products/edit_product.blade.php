            ...
            <form enctype="multipart/form-data" class="form-horizontal" method="post" action="{{ url('admin/edit-product/'.$productDetails->id) }}" name="edit_product" id="edit_product" novalidate="novalidate">{{ csrf_field() }}
              <!-- <div class="control-group">
                ...
                <div class="controls">
                    ...
                    <option value='{{ $category->id }}' {{ $category->id== $productDetails->category_id?'selected':''}}> {{ $category->category_name }}</option>
                    
                    @if(count($category->childs))
                        @include('backend.managechild.managechildcategory',['childs' => $category->childs,'depth'=>1,'category_parent_id'=>$productDetails->category_id])
                    @endif
                  ...
                  <div id="uniform-undefined">
                    <table>
                      <tr>
                        ...
                        <td>
                          @if(!empty($productDetails->image))
                            <img style="width:30px;" src="{{ asset('public/images/backend_images/product/small/'.$productDetails->image) }}"> | <a href="{{ url('/admin/delete-product-image/'.$productDetails->id) }}">Delete</a>
                          @endif
                        </td>
                      </tr>
                    </table>
              ...
              <div class="control-group">
                ...
                <div class="controls">
                  <input type="checkbox" name="status" id="status" @if($productDetails->status == "Active") checked @endif value="1">
                </div>
              </div>
